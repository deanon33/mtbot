...
✨ *All powered by ShazamIO's amazing database!*
    ...
    async def get_artist_info(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not context.args:
            await update.message.reply_text("Please provide an artist name! Usage: /artist \\<artist\\_name\\>")
            return
    ...
    async def search_artists(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not context.args:
            await update.message.reply_text("Please provide an artist name! Usage: /search\\_artist \\<name\\>")
            return
    ...
    async def search_tracks(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not context.args:
            await update.message.reply_text("Please provide a search query! Usage: /search\\_track \\<query\\>")
            return
    ...
    async def get_top_genre_tracks(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not context.args:
            await update.message.reply_text("Please provide a genre! Usage: /top\\_genre \\<genre\\>\\n\\nAvailable genres: POP, ROCK, DANCE, HIP\\_HOP, ALTERNATIVE, COUNTRY, JAZZ, ELECTRONIC")
            return
...
🎸 **Available Genres:**
• POP, ROCK, DANCE, HIP\\_HOP, RAP
• ALTERNATIVE, COUNTRY, JAZZ, ELECTRONIC
